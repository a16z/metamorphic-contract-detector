...
def decode_func_outputs(contract:Contract, function_name:str, return_bytes:bytearray) -> List:
    ...
    func_return_types = [return_type.get('type') for return_type in func.abi.get('outputs')]
    return [decode_abi([*func_return_types], rb) for rb in return_bytes]


def eth_call(provider: Web3, payload: bytearray, return_types:List[str], block_identifier: int = 0) -> Tuple[int,bytearray]:
    """Execute an eth_all
    ...
    if block_identifier != 0:
        res = provider.eth.call({ 'data': payload.hex() }, block_identifier=block_identifier)
    else:
